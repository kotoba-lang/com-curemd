(ns curemd.main
  "Kotodama WASM entrypoint for the Curemd clean-room actor (L4) — Clojure port.
  ...
  facts (`curemd.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "curemd")
(def tier "L4")
...
(def entity-specs
  [{:entity "Patient"      :plural "patients"      :id-prefix "curemd_pat"
    :fields [:mrn :givenName :familyName :birthDate :sex] :required [:mrn :givenName]
    :coerce {}                       :refs {}}
   {:entity "Encounter"    :plural "encounters"    :id-prefix "curemd_enc"
    :fields [:patientId :type :status :startedAt] :required [:type :status]
    :coerce {}                       :refs {:patientId "Patient"}}
   {:entity "Observation"  :plural "observations"  :id-prefix "curemd_obs"
    :fields [:patientId :code :value :unit :takenAt] :required [:code :value]
    :coerce {}                       :refs {:patientId "Patient"}}
   {:entity "Medication"   :plural "medications"   :id-prefix "curemd_med"
    :fields [:patientId :code :dose :route :active] :required [:code :dose]
    :coerce {:active :bool}          :refs {:patientId "Patient"}}
   {:entity "Practitioner" :plural "practitioners" :id-prefix "curemd_pra"
    :fields [:npi :givenName :familyName :specialty] :required [:npi :givenName]
    :coerce {}                       :refs {}}
   {:entity "Appointment"  :plural "appointments"  :id-prefix "curemd_app"
    :fields [:patientId :practitionerId :start :status] :required [:start :status]
    :coerce {}                       :refs {:patientId "Patient" :practitionerId "Practitioner"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"curemd.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "curemd-compat" :tier tier :entities entities} 200])
...
(defn start! [] :curemd-compat/ready)
